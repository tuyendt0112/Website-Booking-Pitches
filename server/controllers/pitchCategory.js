...
const getCategories = asyncHandler(async (req, res) => {
  ...
  queryCommand
    .then(async (response) => {
      const counts = await PitchCategory.find(
        formartedQueries
      ).countDocuments();
...
const updateCategory = asyncHandler(async (req, res) => {
  ...
  const response = await PitchCategory.findByIdAndUpdate(pcid, req.body, {
    new: true,
  });
  return res.json({
    success: response ? true : false,
...
const uploadImagesCategories = asyncHandler(async (req, res) => {
  ...
  const response = await PitchCategory.findByIdAndUpdate(pcid, {
